 ...
 */
public class OpTiledSurface extends Surface{    
    ...
    int viewCols;
    int viewRows;
    JTGLRect clipView = new JTGLRect();
    boolean enableClippedViewPort;
    
    /** Creates a new instance of TiledSurface */
    public OpTiledSurface(int cellRows, int cellColumns, ImageMatrix imageMatrix) {
    ...
    public OpTiledSurface(int cellRows, int cellColumns, JTGLImage image, int frameWidth, int frameHeight) {    
    ...
    public void setMap(short [] map){                    
        System.arraycopy(map,0,cells,0,map.length);
    }
    ...
    public int modSaveCells(int value,int reserveSlots){
	    ...
	    savedCells = new int[numSavedCells][];
            for(int a = 0 ; a < numSavedCells - 1 ; a++){
                ...
                System.arraycopy(oldSlots, 0, savedCells[a], 0, oldSlots.length);
            }
	}
        ...
        while(--i >= 0){
            if(cells[i] == value){                
                if(found >= savedCells[savedIndex].length){
                    ...
                    System.arraycopy(old, 0, newSlot, 0, found);
        ...
        if(found > 0 && found < savedCells[savedIndex].length){
            ...
            System.arraycopy(oldFound, 0, savedCells[savedIndex], 0, found);
        }
        return savedIndex;
    }
    ...
    public void modCells(int savedIndex,int newValue){
        ...
        while(--i >= 0)
            cells[savedIndexes[i]] = (short)newValue;
    ...
    public int createAnimatedTile(int staticIndex) {        
	    ...
	    numAnimated = 1;
            ...
            if(numAnimated == animated.length) {
                ...
                System.arraycopy(animated, 0, tmp_animated, 0, animated.length);
    ...
    public void draw(JTGLGraphics g) {
        if(visible){            
            ...
            int vCols = viewCols;
            int vRows = viewRows;            
            if(cellX + vCols < cellColumns) vCols++;            
            if(cellY + vRows < cellRows) vRows++;                       
            int x0 = x - offsetX;
            int xpos = x0;
            int ypos = y - offsetY;
            int r,c;
            ...
            for(r = 0; r < vRows; r++){
                for(c = 0; c < vCols; c++){ 
                    ...
                    //if(showGrid)
                    //g.drawRect(xpos - offsetX, ypos - offsetY,cellWidth,cellHeight,JTGLColor.RED);     
                    xpos += cellWidth;
                }
                xpos = x0;
                ypos += cellHeight;
