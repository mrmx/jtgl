/*
 * JMappletContextCanvas.java
 *
 * Created on 30 de marzo de 2004, 0:55
 */
 ...
 * @author  Manuel
 */
public class JMappletContextCanvas extends JComponent  implements MappletContext{    
    ...
    /** Creates a new instance of JMappletContextCanvas */
    public JMappletContextCanvas(Container parent) {
        this.parent = parent;
    ...
    public Dimension getPreferredSize() {
        if (dblBuffer != null) {
            ...
            return new Dimension((dblBuffer.getWidth(null)* zoomFactor)
                                 + insets.left
                                 + insets.right,
    ...
    public JTGLImage createJTGLImage(String src) throws JTGLException {
        ...
        if(parent instanceof Applet){
            ...
            JTGLContext.debugMsg("Loading image from url: "+url);
    ...
    public JTGLGraphics getJTGLGraphics(Mapplet mapplet){
        if(gc == null){            
            while(!isVisible() || !isValid())
                synchronized(lock){
                    System.out.println("Waiting for JTGLGraphics");
                    try {                        
                        lock.wait(100);
                    }catch(InterruptedException ex){
                        break;
    ...
    public void handleException(Mapplet mapplet,Throwable ex,String title) {
        if(title != null)
            System.out.println("Exception found "+title);
        ex.printStackTrace();
    }
    ...
    protected void updateCanvas(){        
        ...
        System.out.println("MappletCanvas updated to "+dim.width+"x"+dim.height);
